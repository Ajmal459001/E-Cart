  ...
  return (
    <nav className='flex bg-violet-600 fixed w-full p-5 text-white font-bold'>
      ...
      <ul className='flex-1 text-right'>
        ...
        <li className='list-none inline-block px-5'> <Link to={'/cart'}><i className="fa-solid fa-cart-plus text-green-600"></i>Cart <span className='bg-black rounded text-white p-1'>{userCart?.length}</span></Link> </li>
      </ul>
    </nav>
  )
}
